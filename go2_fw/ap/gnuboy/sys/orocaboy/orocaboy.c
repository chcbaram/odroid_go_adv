...
static int sound = 1;
static uint16_t *sound_buf = NULL;
  ...
  if (pcm.buf)
  {
    ...
    */
    ...
    for (int i=0; i<pcm.pos; i++)
    {
      ...
      out_data = ((int16_t)(pcm.buf[i]-128)) * 0xFF;
      sound_buf[i*2 + 0] = out_data;
      sound_buf[i*2 + 1] = out_data;
    }
    speakerSubmit(sound_buf, pcm.pos);    
  }
  pcm.pos = 0;
  return 1;
}
...
void vid_begin()
{
  //fb.pitch    = lcdGetWidth() * 2;
  //fb.ptr = (byte *)lcdGetFrameBuffer();
}
...
void vid_end()
{
  ...
  switch(vid_mode)
  {
    case 2:
      ...
      //lcdSetSrcWindow(0, 0, 160, 144);
      //lcdSetDstWindow(dest.x, dest.y, dest.w, dest.h);
...
void ev_poll()
{
  if (buttonGetPressedTime(_DEF_HW_BTN_HOME) > 50 && buttonGetPressedEvent(_DEF_HW_BTN_HOME) == true)
  {
    vid_change_mode();
  }
}
...
bool osd_menu(void)
{
  ...
  if (buttonGetPressedTime(_DEF_HW_BTN_MENU) > 50 && buttonGetPressedEvent(_DEF_HW_BTN_MENU) == true)
  {
  }
  else
  {
