...
#define GNUBOY_ROOT_DIR  "/roms/gnuboy"
...
int get_filelist(char *path)
{
  ...
  if (dir != NULL)
  {
    for (;;)
    {
      ent = readdir(dir);                   /* Read a directory item */
      ...
      {                                       /* It is a file. */
        ...
        len = len > 100 ? 100 : len;
        memcpy(flist[i].filename, ent->d_name, len);
        flist[i].filename[len] = 0;
        flist[i].isDir = false;
        flist[i].size = 0; //fno.fsize;
        ++i;
      }
    }
    closedir(dir);
...
int gnuboyFiler()
{
  ...
  lcdSetSrcWindow(0, 0, lcdGetWidth(), lcdGetHeight());
  lcdSetDstWindow(0, 0, lcdGetWidth(), lcdGetHeight());
  ...
  buttonResetTime(_DEF_HW_BTN_MENU);
  return ret;
}
